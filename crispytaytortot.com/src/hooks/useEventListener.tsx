...
export function useEventListenerBody(eventName, handler) {
    if (!isBrowser) {
        return;
    }
    ...
    useEffect(() => {
        // Make sure document.body supports addEventListener
        // On
        const isSupported = document && document.body && document.body.addEventListener;
        if (!isSupported) {
            console.warn(`\`document.body\` does not support \`addEventListener()\`!`);
            return;
        }
        ...
        // Remove event listener on cleanup
        return () => {
            document.body && document.body.removeEventListener(eventName, eventListener);
        };
    }, [eventName, document.body] // Re-run if eventName or document.body changes
    );
}
